...
dtrace:::BEGIN
{
	...
	printf("Sampling SATA error reasons for 10s ...\n");
}
...
/self->bp->b_dip/
{
	statname[args[0]->txlt_sata_pkt] = xlate <devinfo_t *>(self->bp)->dev_statname;
}
...
/args[0]->txlt_sata_pkt->satapkt_cmd.satacmd_cmd_reg/
{
	...
	this->devname = statname[this->sata_pkt] != NULL ? statname[this->sata_pkt] : "<?>";
	this->dir = this->sata_pkt->satapkt_cmd.satacmd_flags.sata_data_direction;
	this->dir_text = sata_dir[this->dir] != NULL ? sata_dir[this->dir] : "<none>";
	this->cmd = this->sata_pkt->satapkt_cmd.satacmd_cmd_reg;
	this->cmd_text = sata_cmd[this->cmd] != NULL ? sata_cmd[this->cmd] : lltostr(this->cmd);
	this->reason = this->sata_pkt->satapkt_reason;
	this->reason_text = sata_reason[this->reason] != NULL ? sata_reason[this->reason] : lltostr(this->reason);
	statname[this->sata_pkt] = 0;
	@[this->devname, this->dir_text, this->cmd_text, this->reason_text] = count();
}
